...
export const GUIDANCE = '本机已安装 dsh-notifications 插件（DSH Web GUI 的「通知」）：当任意会话（含子代理）的审批或计划审批处于等待时，浏览器会播放提示音、在标签页标题显示待审批徽标、右下角出现通知中心（可展开跳转）；会话完成、任务失败、连接掉线或恢复、以及模型/工具运行异常（如 429 配额超限、达到输出上限）时也有提醒。提醒在浏览器端的 dsh Web 页面生效；OS 通知与提示音需浏览器允许；本插件只负责提醒与跳转，不代替用户做批准/拒绝决定。配置可在 GUI 设置页的「插件配置」→「通知」卡片调整。'
